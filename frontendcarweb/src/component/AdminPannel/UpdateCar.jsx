...
import { addCar, updatecar } from "../../actions/carAction";
import { useParams } from "react-router-dom";
import carAddValidationUpdate from "../../utils/formValidationUpdate";
...
const UpdateCar = () => {
  ...
  const getUpdateCar = cars.filter((car) => car._id === id);
  const car = getUpdateCar[0];
  console.log(car);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    let message = carAddValidationUpdate(formData);
    if (message) {
      toast.error(message);
    } else {
      console.log(formData);
